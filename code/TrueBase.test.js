...
const getFolder = () => new TrueBase_1.TrueBaseFolder().setDir(planetsFolderPath).setGrammarDir(planetsFolderPath);
testTree.errorChecking = (equal) => {
...
testTree.fileSystemEvents = async (equal) => {
    // Arrange
    const folder = getFolder();
    folder.loadFolder();
    ...
    waiting = new Promise((resolve) => {
        folder.onDescendantChanged((event) => {
            const fileNode = event.targetNode.getAncestorByNodeConstructor(TrueBase_1.TrueBaseFile);
            // Assert
